...
void initcelllist()
{
    ...
    cz = (int)(zsize - 0.0001) / shellsize / maxdiameter;
            ...
            for (cdz = cellz - 1; cdz < cellz + 2; cdz++)
            {
                ...
                while (p2)
                {
                    if (p2 != p1)
                    {
                        ...
                        if (r2 < rm * rm && fabs(dz) < rm)       //vertical cylinders overlapping condition
                        {
...
double findneighborlistupdate(particle* p1)
{
    ...
    double tz = -1.0 ;
    if (g == 0) {
      if (dvz > 0)  tz = (md - dz) / dvz ;           //time to go out from the top
      else if (dvz < 0)  tz = -(md + dz) / dvz ;     //time to go out from the bottom

    } else {
      if (dvz > 0) {
	disc = dvz * dvz + 2*g*(dz - md) ;
	if (disc>=0)  tz = ( dvz - sqrt(disc) ) / g ; //time to go out from the top
      }
      if (tz < 0) {
	disc = dvz * dvz + 2*g*(dz + md) ;
...
void dumpsnapshot(particle* dumpevent)
{
    ...
    if(dumplogtime == 1) {
      ...
      else createevent(pow(dumplog_base, logstep-1) * (dumplog_base-1.0) * dumplog_dt0, dumpevent, NULL, 102) ;
...
void setparametersfromfile( char * filename )
{
  ...
  if ( !paramfile ) printf("File not found, starting with default parameters\n");
    ...
    while ( myreadline( &buffer , &len , paramfile ) ) {
      ...
      if ( nwords > 0 ) {
	if( ! strcmp( words[0] , "packing_fraction" ) ) sscanf( words[1] , "%lf" , &packfrac ) ;
	...
	else if( ! strcmp( words[0] , "walls" ) ) placeZwalls = 1 ;
