...
double cxsize, cysize; //Cell size
...
void printstuff()
{
    ...
    double press = -dvtot / (2.0 * volume * time) ;
    double pressid = dens ;
...
void randomparticles()
{
    ...
    double vol = N * M_PI / 4 * ( x + (1 - x) * alpha * alpha ) / targetpackfrac ;
    ...
    {
        particles[i].x = 0; particles[i].y = 0 ;
    }
    for (i = 0; i < N; i++)
    {
        p = &(particles[i]);
        ...
        p->next = celllist[p->cellx][p->celly];	//Add particle to celllist
        if (p->next) p->next->prev = p;			//Link up list
        celllist[p->cellx][p->celly] = p;
        p->prev = NULL;
    }
}
...
void randommovement()
{
    ...
    printf("average v2: %lf (%lf, %lf)\n", v2tot / N, vxtot / N, vytot / N);
}
...
void initcelllist()
{
    ...
    printf("Cells: %d, %d\n", cx, cy);
    CEL = (cx > cy) ? cx+1 : cy+1 ;
    celllist = (particle***)calloc(CEL, sizeof(particle**));
    cxsize = xsize / cx;						//Set cell size
    cysize = ysize / cy;
    for (i = 0; i < CEL; i++)	{				//Clear celllist
      celllist[i] = (particle**)calloc(CEL, sizeof(particle*));
      for (j = 0; j < CEL; j++)  celllist[i][j] = NULL;
    }
...
void removefromcelllist(particle* p1)
{
    ...
    else          celllist[p1->cellx][p1->celly] = p1->next;
...
void addtocelllist(particle* p)
{
    ...
    celllist[p->cellx][p->celly] = p;
    p->prev = NULL;
    p->edge = (p->cellx == 0 || p->celly == 0 || p->cellx == cx - 1 || p->celly == cy - 1);
		...
		if (p2 != p1)
		  {
		    ...
		    r2 = dx * dx + dy * dy;
		    rm = (p1->r + p1->vr*maxdt + p2->rtarget) * shellsize;
		    if (r2 < rm * rm)
		      {
...
void collision(event* ev)
{
    ...
    double b = dx * dvx + dy * dvy - dvr;                  //dr.dv
    b *= 1.0 / (m1 + m2);
    double dv1 = 2 * b * m2;
    double dv2 = 2 * b * m1;
    dvtot += 2 * m1 * m2 * b;
...
void write()
{
    ...
    double temperature = 0.5 * en / (float)N ;
    ...
    double press = -dvtot / (2.0 * volume * timeint);
    double pressid = dens;
    ...
    if (makesnapshots && time - lastsnapshottime > snapshotinterval - 0.001)
    {
        ...
        for (i = 0; i < N; i++)
        {
            ...
            fprintf(file, "%c %.12lf  %.12lf 0.0  %lf\n", 'a' + p->type, p->x + xsize * p->boxestraveledx, p->y + ysize * p->boxestraveledy, p->r);
        }
        fclose(file);
...
void checkcells()
{
    ...
    for (x = 0; x < cx; x++)
        for (y = 0; y < cy; y++)
	  {
	    ...
	    if (p)
	      {
		if (p->prev) printf("First part has a prev (%d, %d) %d, %d\n",
				    x, y, p->number, p->prev->number);
		while (p)
		  {
		    count++;
		    if (p->cellx != x || p->celly != y)
		      {
			printf("Cell error: %d, %d / %d, %d\n", x, y, p->cellx, p->celly);
			exit(3);
		      }
...
void writelast()
{
    ...
    {
        p = &(particles[i]);
        fprintf(file, "%c %.12lf %.12lf 0.0 %.12lf\n", 'a' + p->type, p->x, p->y, p->r);
    }
    fclose(file);
}
